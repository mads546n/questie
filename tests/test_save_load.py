...
import json
import os
from main import save_data, load_data, weekly_planner, reset_planner
...
class TestSaveAndLoadData(unittest.TestCase):
    ...
    def setUp(self):
        ...
        reset_planner()
        # Associate global weekly_planner with sample data
        global weekly_planner
        weekly_planner = self.sample_data.copy()
    ...
    def test_load_data_with_empty_file(self, mock_file):
        # Simulation of an empty file
        mock_file.return_value.read.return_value = ""
        load_data()
        # Ensure that the default structure is loaded
        self.assertEqual(weekly_planner["Monday"], [])
        self.assertEqual(weekly_planner["metadata"]["completed_quests_count"], 0)
    ...
    def test_integration_save_and_load(self):
        ...
        with patch("builtins.open", new_callable=mock_open, read_data=json.dumps(self.sample_data)):
            load_data()
    ...
    def test_reset_planner(self):
        ...
        self.assertEqual(weekly_planner, expected_reset_data)
